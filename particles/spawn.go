...
func (s *System) SpawnUp() { // cette fonction met à jours le systeme qui indique combien de nouvelles particules qui seront engendrées
	...
	s.tick++
	if config.General.SpawnRate < 1 || config.General.SpawnRate > 0 { // si la valeur du tick qui sera engendree est inferieur à 1 alors on engendra une particule sur deux tick
